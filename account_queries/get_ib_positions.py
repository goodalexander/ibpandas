...
def output_position_df_map(connection_ip="127.0.0.1",port_id=7496, client_id=1):
    pos_list=''
    accountName=''
    acct_val=''
    acct_upd=''

    app = TestApp(connection_ip, port_id, client_id)
    ...
    #acct_upd = accounting_updates
    app.disconnect()
    def output_stock_position_df():
        temp_df=pd.DataFrame(pos_list)
        temp_df.columns = ['account', 'contract', 'position','pnl']
        temp_df['security_id']=temp_df.contract.apply(lambda x: str(x).split(',')[0])
        temp_df['ticker']=temp_df.contract.apply(lambda x: str(x).split(',')[1])
        temp_df['position_type']=temp_df.contract.apply(lambda x: str(x).split(',')[2])
        temp_df['exchange']= temp_df.contract.apply(lambda x: str(x).split(',')[7])
        stock_positions=temp_df[temp_df.position_type == 'STK']
        return stock_positions
    def output_fx_position_df():
        acct_val_df=pd.DataFrame(acct_val)
        cash_df=acct_val_df[acct_val_df[0]=='CashBalance']
        cash_df.columns = ['position_type','balance','currency']
        cash_df=cash_df[cash_df['currency'] != 'BASE']
        return cash_df

    account_value_df= pd.DataFrame(accounting_values)
    account_value_df.columns = ['field','value','currency']

    output_map= {}
    output_map['stock_positions']= output_stock_position_df()
    output_map['fx_positions'] = output_fx_position_df()
    output_map['account_values'] = account_value_df
    return output_map
